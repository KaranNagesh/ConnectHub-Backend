...
public class RoomAvatarService {
    ...
    @Value("${connecthub.room-avatar.upload-dir:uploads/room-avatars}")
    private String uploadDir;

    public String saveAvatar(MultipartFile file, String roomId) throws IOException {
        validate(file);
        ...
        String safeRoomId = roomId.replaceAll("[^a-zA-Z0-9-]", "");
        String filename = "room-" + safeRoomId + "-" + UUID.randomUUID() + EXTENSIONS.get(contentType);
        Path target = dir.resolve(filename).normalize();
        if (!target.startsWith(dir)) {
            throw new BadRequestException("Invalid room photo filename");
        }

        file.transferTo(target);

        return ServletUriComponentsBuilder.fromCurrentContextPath()
                .path("/api/v1/rooms/avatars/")
                .path(filename)
                .toUriString();
    }
    ...
    public AvatarResource loadAvatar(String filename) throws MalformedURLException {
        ...
        if (!file.startsWith(dir) || !Files.exists(file) || !Files.isRegularFile(file)) {
            throw new BadRequestException("Room photo not found");
        }
    ...
    private void validate(MultipartFile file) {
        ...
        if (file.getSize() > MAX_AVATAR_SIZE) {
            throw new BadRequestException("Group photo must be 5MB or smaller");
        }
        String contentType = file.getContentType();
